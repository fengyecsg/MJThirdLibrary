...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _indicatorInsets = UIEdgeInsetsMake(5, 15, 5, 15);
        _indicatorAligment = SMPageControlAlignmentCenter;
        _indicatorVerticalAlignment = SMPageControlVerticalAlignmentBottom;
    }
    return self;
}
...
- (void)initializePageControl {
    _pageControl = [[SMPageControl alloc] initWithFrame:CGRectMake(_indicatorInsets.left,
                                                                   _indicatorInsets.top,
                                                                   self.width - (_indicatorInsets.left + _indicatorInsets.right),
                                                                   self.height - (_indicatorInsets.top + _indicatorInsets.bottom))];
    _pageControl.verticalAlignment = _indicatorVerticalAlignment;
    _pageControl.alignment = _indicatorAligment;
    _pageControl.userInteractionEnabled = NO;
    _pageControl.hidesForSinglePage = YES;
    _pageControl.indicatorDiameter = 5;
    _pageControl.indicatorMargin = 7;
    [self addSubview:_pageControl];
}

- (SMPageControl *)bannerPageControl {
    return _pageControl;
}
...
- (void)loadData {
    ...
    for (NSInteger i = 0; i < _datasourceImages.count; i++) {
        ...
        if ([imageSource isKindOfClass:[UIView class]]) {
            ((UIView *)imageSource).frame = CGRectMake(contentWidth * i, 0, ((UIView *)imageSource).frame.size.width, ((UIView *)imageSource).frame.size.height);
            ((UIView *)imageSource).center = CGPointMake(contentWidth * i + contentWidth/2.0, contentHeight/2.0);
            UIView *view = [self duplicate:imageSource];
            view.frame = ((UIView *)imageSource).frame;
