...
{
    presentingWindow = AppDelegate.window;
    if (_topWindow != AppDelegate.window && _topWindow != nil) {
        [[LNWindowManager sharedWindowManager].topWindow.presentingWindow dismissWindow:[LNWindowManager sharedWindowManager].topWindow animated:NO completion:nil];
    }
    if(presentingWindow.isKeyWindow)
    {
        [presentingWindow endEditing:YES];
    }
...
//	[presentedWindow.rootViewController viewDidAppear:NO];
    ...
    presentedWindow.windowLevel = UIWindowLevelNormal ;
