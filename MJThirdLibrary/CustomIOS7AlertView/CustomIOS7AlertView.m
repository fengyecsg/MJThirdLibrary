...
- (id)init
{
    ...
    if (self) {
        ...
        buttonTitles = @[@"Close"];
        self.buttonsArray = [NSMutableArray arrayWithCapacity:INIT_PARAMETER_ARRAY_CAPACITY];
...
{
    DDLogInfo(@"Button Clicked! %d, %d", (int)buttonIndex, (int)[alertView tag]);
    [self close];
}
...
- (UIView *)createContainerView
{
    ...
    self.gradientLayer.frame = dialogContainer.bounds;
    ...
    [dialogContainer.layer insertSublayer:self.gradientLayer atIndex:0];
    ...
    lineView.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:0.5f];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if (i < [buttonTitles count]-1){
            UIView *lineSperator = [[UIView alloc] initWithFrame:CGRectMake(closeButton.frame.size.width-1, 0, 0.5, closeButton.frame.size.height)];
            lineSperator.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
            [closeButton addSubview:lineSperator];
        }
