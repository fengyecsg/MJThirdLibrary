...
- (void)layoutSubviews {
  ...
  frame.origin.x = self.contentView.frame.size.width - frame.size.width - frame.origin.y - DISTANCE(7);
...
- (void)switchDidChangeValue:(UISwitch *)swich {
  NISwitchFormElement* switchElement = (NISwitchFormElement *)self.element;
    if (switchElement.value == _switchControl.on) {
        return;
    }
